    ...
    def on_login_pressed():
        ...
        def poll():
            ...
            if result == "wait":
                ...
                def poll_fetch():
                    status_label.config(text="Fetching account data...")

                    fetch_result = backend.fetch_from_server()

                    if fetch_result == "wait":
                        root.after(100, poll_fetch)

                    elif fetch_result == "done":
                        show_item_screen()

                    else:
                        status_label.config(
                            text=f"fetch failed: {fetch_result}"
                        )
...
def show_item_screen():
    ...
    create_button = tk.Button(
        current_screen,
        text="Create Item",
        command=show_create_item_screen
    )
    create_button.pack()
    ...
    if isinstance(my_items, list):
        ...
        for item_idx, item in enumerate(my_items):
            ...
            tk.Label(
                items_frame,
                text=backend.item_name(item),
                font=("Arial", 8)
            ).grid(row=row, column=0, padx=5)

            tk.Label(
                items_frame,
                text=f"{backend.item_size(item)} bytes.",
                font=("Arial", 8)
            ).grid(row=row, column=1, padx=5)

            tk.Label(
                items_frame,
                text=backend.item_encryption_method(item),
                font=("Arial", 8)
            ...
            tk.Button(
                items_frame,
                text="Invite",
                command=lambda item=item: show_invite_screen(item)
            ...
            tk.Button(
                items_frame,
                text="Leave",
                command=lambda item=item: show_leave_screen(item)
            ).grid(row=row, column=6, padx=5)

            tk.Button(
                items_frame,
                text="Delete",
                command=lambda item=item: show_delete_screen(item)
            ).grid(row=row, column=7, padx=5)
    ...
    if isinstance(invite_items, list):
        ...
        for item_idx, item in enumerate(invite_items):
            ...
            tk.Button(
                invites_frame,
                text="Join",
                command=lambda item=item: show_join_screen(item)
            ).grid(row=row, column=3, padx=5)

            tk.Button(
                invites_frame,
                text="Reject",
                command=lambda item=item: show_reject_screen(item)
            ).grid(row=row, column=4, padx=5)

    status_label.config(text="")


def show_download_screen(item: Uuid):
    global current_screen
    ...
    exit_button = tk.Button(
        current_screen,
        text="X",
        command=show_item_screen
    )
    exit_button.place(relx=0.98, rely=0.02, anchor="ne")

    status_label = tk.Label(current_screen, text="Downloading item...")
    status_label.pack()

    def poll():
        status_label.config(text="Downloading item...")

        result = backend.read_item(item)
        ...
        if isinstance(result, bytes):
            try:
                save_path = fd.asksaveasfilename(
                    initialfile=backend.item_name(item)
                )
                ...
                status_label.config(text=f"Download failed: {exp}")

            return

        # handle error cases from backend.read_item
        status_label.config(text=f"Download failed: {result}")
...
def show_invite_screen(item: Uuid):
    ...
    def on_invite_pressed():
        ...
        def poll_invite():
            status_label.config(
                text=f"Inviting {selected_email}..."
            )

            exit_button.config(state="disabled")
            invite_button.config(state="disabled")

            result = backend.invite_user_to_item(
                selected_email,
                item
            )
            ...
            if result == "wait":
                ...
                status_label.config(
                    text=f"User {selected_email} invited!"
                )
...
def show_release_item_screen(item: Uuid):
    ...
    until_label = tk.Label(
        current_screen,
        text="Until Date (YYYY-MM-DD HH:MM)"
    )
    until_label.pack()
    ...
    def on_release_pressed():
        ...
        def poll():
            ...
            if result == "wait":
                ...
                status_label.config(
                    text=f"Error releasing item: {result}"
                )
...
def show_create_item_screen():
    ...
    def on_create_pressed():
        ...
        try:
            ...
            status_label.config(
                text=f"Failed to read file: {exp}"
            )
            return
...
def show_leave_screen(item: Uuid):
    ...
    status_label = tk.Label(
        current_screen,
        text="Leaving and Releasing Item..."
    )
    status_label.pack()
    ...
    def poll():
        ...
        if result == "wait":
            ...
            status_label.config(
                text=f"error leaving item: {result}"
            )
...
def show_delete_screen(item: Uuid):
    ...
    def poll():
        ...
        if result == "wait":
            ...
            status_label.config(
                text=f"error deleting item: {result}"
            )
...
def show_join_screen(item: Uuid):
    ...
    def poll():
        ...
        if result == "wait":
            ...
            status_label.config(
                text=f"error joining item: {result}"
            )
...
def show_reject_screen(item: Uuid):
    ...
    def poll():
        ...
        if result == "wait":
            ...
            status_label.config(
                text=f"error rejecting item: {result}"
            )
